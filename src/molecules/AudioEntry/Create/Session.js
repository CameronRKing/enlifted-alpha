...
import Entry from '../../../models/AudioEntry.js';
...
export default class Session {
    ...
    async toggle() {
        if (this.isRecording) {
            ...
            this.startRecordManualPause();
            // pauses leading up to the finish are ignored
            if (this.pauseTracker.isTracking) this.pauseTracker.popLast();
        } else {
            if (!this.manualPauses.length) this.startTimestamp = DateTime.local();
            this.endRecordManualPause();
    ...
    startRecordManualPause() {
        this.manualPauses.push(Date.now());
    }

    endRecordManualPause() {
        const last = this.manualPauses.pop();
        if (!last) return;
        this.manualPauses.push(Duration.fromMillis(Date.now() - last));
    }
    ...
    async finish() {
        ...
        const length = this.manualPauses.filter(pause => typeof pause !== 'number')
            .reduce((acc, pause) => acc.minus(pause), Duration.fromMillis(ms))
        ...
        const entry = new Entry(
            ...
            new AudioFile(
                await this.audioInput.getAudio(),
                length,
                wpm,
